...
import { Trophy, Shield, Users, Star, Award, X } from 'lucide-react';
...
export default function Leaderboard({ collection, lang, user }) {
  ...
  const userSquadStats = useMemo(() => {
    ...
    if (players.length < 11) {
      return { name: user?.username || 'My Squad', ovr: 0, chem: 0, isUser: true, incomplete: true };
    }
  ...
  const rankings = useMemo(() => {
    ...
    // Always include user squad in rankings
    list.push(userSquadStats);
    
    // Sort by OVR (highest first), then Chem (highest first)
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      <div style={{ marginBottom: '2rem', textAlign: 'center' }}>
        <h2 className="glow-gold" style={{ fontSize: '2rem', fontWeight: '950', color: 'var(--accent-gold)', textTransform: 'uppercase', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
          <Trophy size={28} />
          <span>{t.leaderboard || 'Leaderboard'}</span>
        </h2>
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem', marginTop: '0.25rem' }}>
          {lang === 'tr' 
            ? 'Kurduğun kadroyla demo takımlara meydan oku ve sıralamada zirveye tırman!' 
            : 'Challenge demo teams with your custom squad and climb the global rankings!'}
        </p>
      </div>
        ...
        <div className="modal-overlay" onClick={() => setActiveDetailsTeam(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '500px' }}>
            ...
            <div className="modal-body" style={{ padding: '1.25rem' }}>
              ...
              {/* Player list of the selected team */}
