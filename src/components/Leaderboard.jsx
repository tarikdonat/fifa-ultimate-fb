...
import { Trophy, Shield, Star, Award, X, Play, RefreshCw, Sparkles, Volume2, VolumeX } from 'lucide-react';
...
// Synthesize referee whistle and crowd noise using Web Audio API
const playSynthesizedSound = (type, volumeEnabled) => {
  ...
  try {
    ...
    if (type === 'whistle') {
      // 2 short high pitch chirps + 1 long chirp
      ...
      // Stadium Goal Cheer: White noise with filter sweeps simulating crowd roar
  ...
  const flags = {
    ...
    argentina: (
      <svg viewBox="0 0 3 2" style={{ width: '100%', height: '100%', display: 'block' }}>
        ...
        <rect y="1.34" width="3" height="0.67" fill="#74ACDF" />
    ...
    uruguay: (
      <svg viewBox="0 0 3 2" style={{ width: '100%', height: '100%', display: 'block' }}>
        ...
        <rect width="1.2" height="0.88" fill="#FFFFFF" />
...
export default function Leaderboard({ collection, lang, user, coins, onUpdateCoins }) {
  ...
  // Derby Match Simulator States
  const [isDerbyOpen, setIsDerbyOpen] = useState(false);
  const [derbyStep, setDerbyStep] = useState('setup'); // setup, playing, result
  const [homeTeam, setHomeTeam] = useState(DEMO_TEAMS[0]);
  const [awayTeam, setAwayTeam] = useState(DEMO_TEAMS[1]);
  const [selectedSide, setSelectedSide] = useState('home'); // 'home' (represent Home) or 'away' (represent Away)
  ...
  // UPGRADED STATES FOR MATCH ARCADE ENGINE
  const [soundEnabled, setSoundEnabled] = useState(true);
  const [userTactic, setUserTactic] = useState('balanced'); // balanced, attacking, defensive
  const [opponentTactic, setOpponentTactic] = useState('balanced');
  const [isHalftime, setIsHalftime] = useState(false);
  const [triggeredMinutes, setTriggeredMinutes] = useState([]);
  const [ballPosition, setBallPosition] = useState('midfield'); // midfield, home_attack, away_attack, home_goal, away_goal
  ...
  const [duelResult, setDuelResult] = useState(null); // 'goal', 'saved', 'missed'
  const [duelStep, setDuelStep] = useState(null); // 'intro', 'clash', 'result'
  ...
  // Filter list of available squads for derby match
  ...
  const handleStartDerby = () => {
    ...
    // Choose opponent tactic randomly
    ...
    // Setup initial possession based on strategies
  ...
  // Live simulation tick hook
  useEffect(() => {
    let interval = null;
    if (isDerbyOpen && derbyStep === 'playing' && !duelStep && !criticalMoment && !isHalftime) {
      interval = setInterval(() => {
        setSimTimer(prev => {
          const increment = Math.floor(Math.random() * 2) + 2; // tick 2-3 mins at a time
      ...
      }, 420); // clock ticks every 420ms
    }
    return () => { if (interval) clearInterval(interval); };
  }, [isDerbyOpen, derbyStep, duelStep, criticalMoment, isHalftime, homeTeam, awayTeam, soundEnabled]);
  ...
  const triggerScheduleOrCommentary = (minute) => {
    ...
    // Card Duel Attacking event triggers
    ...
    // First Half Critical moment trigger
    ...
    // Second Half Critical moment trigger
  ...
  const startCardDuel = (minute) => {
    ...
    const attackingSide = isHomeAttacking ? 'home' : 'away';
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        const luck = Math.floor(Math.random() * 7) - 3; // -3 to +3
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      {/* Dynamic inline styles for the pitch, cards, and transitions */}
      <style>{`
        .derby-pitch {
          width: 100%;
          height: 180px;
          background: radial-gradient(circle, #155e37 0%, #064e2b 100%);
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div style={{ flexGrow: 1 }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', marginTop: '0.2rem' }}>
            {lang === 'tr' 
              ? 'Kurduğun kadroyla efsane takımlara meydan oku!' 
              : 'Challenge legend squads with your custom FUT team!'}
          </p>
        </div>

        {/* Derby Trigger Button */}
        <button 
          className="btn-primary" 
          onClick={() => setIsDerbyOpen(true)}
          style={{ 
        ...
        >
          <Play size={16} />
          <span>{lang === 'tr' ? 'Derbi Maçı Yap!' : 'Play Derby Match!'}</span>
        </button>
      </div>
        ...
        <div className="modal-overlay" style={{ zIndex: 1200 }}>
          <div className="modal-content" style={{ maxWidth: '600px', background: 'radial-gradient(circle at top, #111827 0%, #030712 100%)', border: '1px solid var(--accent-gold)' }}>
            ...
            <div className="modal-header" style={{ borderBottom: '1px solid var(--border-color)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <h3 style={{ fontSize: '1.2rem', fontWeight: '900', color: 'var(--accent-gold)', textTransform: 'uppercase', margin: 0 }}>
                  {lang === 'tr' ? 'Derbi Simülasyonu' : 'Derby Simulation'}
                </h3>
              </div>
              {derbyStep !== 'playing' && (
                <button onClick={handleCloseDerby} style={{ background: 'transparent', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer' }}>
                  <X size={20} />
                </button>
              )}
            </div>
            ...
            <div className="modal-body" style={{ padding: '1.25rem' }}>
              ...
              {derbyStep === 'setup' && (
                <div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.25rem' }}>
                    {/* Home Team Selector */}
                    <div>
                      <label className="form-label">{lang === 'tr' ? '1. Takım (Ev Sahibi)' : 'Home Team'}</label>
                      <select 
                    ...
                    {/* Away Team Selector */}
                    <div>
                      <label className="form-label">{lang === 'tr' ? '2. Takım (Deplasman)' : 'Away Team'}</label>
                      <select 
                        className="form-input"
                        value={awayTeam.id}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1rem', marginBottom: '1.5rem', backgroundColor: 'var(--bg-secondary)', padding: '0.75rem', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                    <div>
                      <label className="form-label" style={{ marginBottom: '0.4rem', fontSize: '0.7rem' }}>
                      ...
                      <div style={{ display: 'flex', gap: '0.3rem' }}>
                        {['balanced', 'attacking', 'defensive'].map(tac => (
                          ...
                          >
                            {tac === 'balanced' ? (lang === 'tr' ? 'Dengeli' : 'Bal') :
                             tac === 'attacking' ? (lang === 'tr' ? 'Hücum' : 'Att') :
                             (lang === 'tr' ? 'Savunma' : 'Def')}
                          </button>
                        ))}
                      </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', borderLeft: '1px solid var(--border-color)', paddingLeft: '0.5rem' }}>
                      <span className="form-label" style={{ fontSize: '0.65rem', marginBottom: '0.2rem' }}>
                        {lang === 'tr' ? 'SES' : 'SOUND'}
                      </span>
                      <button
                        className="btn-secondary"
                  ...
                  <button 
                    ...
                    style={{ width: '100%', justifyContent: 'center', background: 'linear-gradient(135deg, #10b981, #059669)', color: '#fff', boxShadow: 'none' }}
                  >
                    <Play size={16} />
              ...
              {/* STEP 2: Simulation playing */}
              {derbyStep === 'playing' && (
                <div>
                  
                  {/* Scoreboard */}
                  <div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', backgroundColor: 'var(--bg-secondary)', padding: '1rem', borderRadius: '10px', border: '1px solid var(--border-color)', marginBottom: '1rem', position: 'relative' }}>
                    
                    {/* Sound Switcher */}
                    <button 
                      onClick={() => setSoundEnabled(prev => !prev)}
                      style={{ position: 'absolute', right: '10px', top: '10px', background: 'transparent', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer' }}
                    >
                      {soundEnabled ? <Volume2 size={16} /> : <VolumeX size={16} />}
                    </button>
                    ...
                    <div style={{ textAlign: 'center', width: '38%' }}>
                      ...
                      <span style={{ fontSize: '1.05rem', fontWeight: '900', display: 'block', marginTop: '0.25rem', color: homeTeam.color }}>
                        {homeTeam.name}
                      </span>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: '80px' }}>
                      <span className="glow-gold" style={{ fontSize: '0.85rem', color: 'var(--accent-gold)', fontWeight: '900', letterSpacing: '1px' }}>
                        {simTimer}'
                      </span>
                      <span style={{ fontSize: '2rem', fontWeight: '900', letterSpacing: '2px', color: 'var(--text-primary)', margin: '0.1rem 0' }}>
                        {homeScore} - {awayScore}
                      </span>
                    </div>
                    ...
                    <div style={{ textAlign: 'center', width: '38%' }}>
                      ...
                      <span style={{ fontSize: '1.05rem', fontWeight: '900', display: 'block', marginTop: '0.25rem', color: awayTeam.color }}>
                        {awayTeam.name}
                      </span>
                      <span style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', textTransform: 'uppercase' }}>
                    ...
                    <div 
                      ...
                      style={{ 
                        ...
                        top: ballPosition === 'midfield' ? '50%' :
                             ...
                             ballPosition === 'home_goal' ? '3%' : '97%',
                        transform: 'translate(-50%, -50%)'
                      }}
                    />

                    {/* Team logos printed on pitch zones */}
                    <div style={{ position: 'absolute', top: '15px', left: '15px', color: 'rgba(255,255,255,0.15)', fontWeight: 'bold', fontSize: '1.5rem', textTransform: 'uppercase' }}>
                      {homeTeam.logo}
                    </div>
                    <div style={{ position: 'absolute', bottom: '15px', right: '15px', color: 'rgba(255,255,255,0.15)', fontWeight: 'bold', fontSize: '1.5rem', textTransform: 'uppercase' }}>
                      {awayTeam.logo}
                    </div>
                  ...
                  <div style={{ fontSize: '0.9rem', color: 'var(--text-secondary)' }}>
                    ...
                    <div style={{ height: '180px', overflowY: 'auto', backgroundColor: '#030712', border: '1px solid var(--border-color)', borderRadius: '8px', padding: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                      {derbyLog.map((log, index) => {
                        ...
                        return (
                          <div 
                            key={index} 
                            style={{ 
                              borderLeft: `2.5px solid ${isGoal ? 'var(--accent-gold)' : isWhistle ? '#3b82f6' : index === 0 ? '#10b981' : 'transparent'}`,
                              paddingLeft: '0.5rem',
                          ...
                          >
                            <span style={{ color: 'rgba(255,255,255,0.35)', marginRight: '0.4rem', fontFamily: 'monospace' }}>[{log.minute}']</span>
                            <span>{log.event}</span>
                          </div>
                        );
                      })}
              ...
              {derbyStep === 'result' && (
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: '2.5rem', fontWeight: '900', letterSpacing: '4px', margin: '0.5rem 0' }}>
                    {homeScore} - {awayScore}
                  </div>
                  ...
                  {/* POST-MATCH STATISTICS CARD */}
                  <div className="glass-panel" style={{ maxWidth: '400px', margin: '0 auto 1.25rem auto', border: '1px solid var(--border-color)', padding: '0.75rem', fontSize: '0.75rem' }}>
                    <h4 style={{ fontWeight: '900', color: 'var(--accent-gold)', textTransform: 'uppercase', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.4rem', marginBottom: '0.5rem' }}>
                      {lang === 'tr' ? 'MAÇ İSTATİSTİKLERİ' : 'MATCH STATISTICS'}
                    </h4>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span style={{ fontWeight: 'bold' }}>{homeTeam.logo}</span>
                        <span style={{ color: 'var(--text-secondary)', fontWeight: 'bold' }}>TEAM</span>
                        <span style={{ fontWeight: 'bold' }}>{awayTeam.logo}</span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span>{matchStats.possessionHome}%</span>
                        <span style={{ color: 'var(--text-secondary)' }}>{lang === 'tr' ? 'Topa Sahip Olma' : 'Possession'}</span>
                  ...
                  {/* Reward Card */}
                  <div className="glass-panel" style={{ maxWidth: '300px', margin: '0 auto 1.25rem auto', border: '1px solid var(--accent-gold)', padding: '0.75rem' }}>
                    <span style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', display: 'block', fontWeight: '800', textTransform: 'uppercase', letterSpacing: '1px' }}>
                      {lang === 'tr' ? 'DERBİ REYTİNG ÖDÜLÜ' : 'DERBY REWARD COINS'}
                    </span>
                    <span style={{ fontSize: '2rem', fontWeight: '900', color: 'var(--accent-gold)', display: 'block', margin: '0.15rem 0' }}>
                      🪙 {rewardCoins}
                    </span>
